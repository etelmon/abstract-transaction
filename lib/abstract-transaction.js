...
class Transaction {
    ...
    startInner(options) {
        return defineTransaction('REUSE', this.handler, options);
    }
...
function defineTransaction(requirement, parentTransactionHandler, options) {
    ...
    const transaction = new Impl(parentTransaction, options);
...
function setTransactionImplementationClass(implClass) {
    DefaultTransactionImplementation = implClass;
}
